...
impl WebDriverClient {
    ...
    pub async fn press_enter(&self, session_id: &str) -> Result<()> {
        ...
        let active_id = self.active_element(session_id).await?;
        self.type_element(session_id, &active_id, "\n").await
    }

    pub async fn active_element(&self, session_id: &str) -> Result<String> {
        ...
        active_response
            ...
            .ok_or_else(|| anyhow!("no active element available"))
    }
    ...
    pub async fn tap_point(&self, session_id: &str, x: f64, y: f64) -> Result<()> {
        self.tap_point_with_options(session_id, x, y, false).await
    }

    pub async fn tap_point_with_options(
        &self,
        session_id: &str,
        x: f64,
        y: f64,
        ...
        if !x.is_finite() || !y.is_finite() {
            bail!("tap point must be finite, got x={x}, y={y}");
        }
        if !allow_out_of_bounds {
            let (width, height) = self.window_rect(session_id).await?;
            if x < 0.0 || y < 0.0 || x > width || y > height {
                bail!(
                    "tap point outside viewport: x={x}, y={y}, viewport={width}x{height}; set allowOutOfBounds=true only for calibrated device coordinates"
                );
            }
        }
        let payload = json!({
...
mod tests {
    ...
    #[tokio::test]
    async fn runtime_guardrails_tap_point_rejects_out_of_viewport_coordinates() {
        let server = MockServer::start_async().await;
        let rect_mock = server
            .mock_async(|when, then| {
                when.method(GET).path("/session/sess-1/window/rect");
                then.status(200)
                    .json_body(json!({"value": {"width": 100, "height": 200}}));
            })
            .await;

        let client = WebDriverClient::new(&server.url("")).expect("client");
        let err = client
            .tap_point("sess-1", 101.0, 20.0)
            .await
            .expect_err("bounds error");

        rect_mock.assert_async().await;
        assert!(err.to_string().contains("outside viewport"));
    }

    #[tokio::test]
    async fn runtime_guardrails_tap_point_allows_explicit_out_of_bounds_override() {
        let server = MockServer::start_async().await;
        let action_mock = server
            .mock_async(|when, then| {
                when.method(POST).path("/session/sess-1/actions");
                then.status(200).json_body(json!({"value": null}));
            })
            .await;
        ...
        let client = WebDriverClient::new(&server.url("")).expect("client");
        client
            .tap_point_with_options("sess-1", 999.0, 999.0, true)
            .await
            .expect("tap");
