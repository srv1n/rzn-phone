...
Local history and favorites live under `${XDG_STATE_HOME:-$HOME/.local/state}/rzn-phone`. Remove them too when you want a full local cleanup:
...
rm -rf "${XDG_STATE_HOME:-$HOME/.local/state}/rzn-phone" # optional: removes local history and favorites
```
...
- Short aliases: `rzn-phone show <ref>` shows either a workflow or a tool.
...
- `--fast 1` enables reuse behavior for one run.
...
- Workflow format: [schema/rzn-mobile-workflow.schema.json](schema/rzn-mobile-workflow.schema.json) and [workflow guide](docs/system/workflows.md)
- Agent setup guide: [docs/system/setup.md](docs/system/setup.md)
- Current architecture and app workflow notes: [docs/system/architecture.md](docs/system/architecture.md) and [docs/system/workflows.md](docs/system/workflows.md)
