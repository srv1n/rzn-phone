...
run_workflow_rpc() {
  ...
  req_file="$(mkrepo_tmpfile "rzn-phone-workflow-rpc")"
...
JSON
  ...
  messages-find-otp)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "messages-find-otp")"
    fi
    mkdir -p "$OUT_DIR"
...
JSON
  ...
  reddit-daily-scroll)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-daily-scroll")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-open-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-open-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-like-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-like-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-comment-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-comment-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-reply-comment)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-reply-comment")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-open-inbox)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-open-inbox")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-open-dm-thread)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-open-dm-thread")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-send-dm)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-send-dm")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-send-dm-user)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-send-dm-user")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-reply-dm)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-reply-dm")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  reddit-engage-seq)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "reddit-engage-seq")"
    fi
    mkdir -p "$OUT_DIR"
...
JSON
  ...
  appstore-typeahead)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "appstore-typeahead")"
    fi
    mkdir -p "$OUT_DIR"
...
JSON
  ...
  appstore-search-results)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "appstore-results")"
    fi
    mkdir -p "$OUT_DIR"
...
JSON
  ...
  appstore-smoke)
    ...
    OUT_ROOT="$(mkrepo_tmpdir "appstore-smoke")"
  ...
  linkedin-read-feed)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-read-feed")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  google-maps-place)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "google-maps-place")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  google-maps-directions)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "google-maps-directions")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  google-maps-start)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "google-maps-start")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-open-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-open-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-like-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-like-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-comment-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-comment-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-reply-comment)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-reply-comment")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-create-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-create-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-update-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-update-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-delete-post)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-delete-post")"
    fi
    mkdir -p "$OUT_DIR"
  ...
  linkedin-daily-scroll)
    ...
    if [[ -z "$OUT_DIR" ]]; then
      OUT_DIR="$(mkrepo_tmpdir "linkedin-daily-scroll")"
    fi
    mkdir -p "$OUT_DIR"
