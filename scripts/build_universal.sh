...
cargo build -p "$CRATE" --release --target aarch64-apple-darwin --bin rzn-phone --bin rzn-phone-worker

echo "[build] building ${CRATE} for x86_64-apple-darwin"
cargo build -p "$CRATE" --release --target x86_64-apple-darwin --bin rzn-phone --bin rzn-phone-worker
