# Releasing RZN Phone Automation (`rzn-phone`)
...
flowchart LR
  ...
  G --> Q["Pre-package gate: fmt, clippy, tests, Python compile, workflow metadata, pack smoke"]
...
This repo is licensed under the GNU Affero General Public License v3.0 only (`AGPL-3.0-only`).
Public without a license is source-visible, not open source. Ship the `LICENSE` file with release artifacts.
