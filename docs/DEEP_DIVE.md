# RZN Phone Automation Deep Dive

`rzn-phone` is the repo slug and command surface. The human-facing capability name is `RZN Phone Automation`, and the compact diagram label is `Phone Automation`.
...
2. Run it via `rzn-phone run ...` against a real device or host dev-mount
...
5. **Validate**: run a representative `rzn-phone run ...` smoke on a device, then publish the JSON pack.
